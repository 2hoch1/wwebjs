...
<Callout type="idea">
  To check if you already have Node installed on your machine, run `node -v` in your **terminal**.
  If the output is `v18` or higher, then you're good to go! Otherwise you should continue reading.
</Callout>

<Callout type="error">
  If you already have Node installed, but you are using an older version that is below v18, you need
  to upgrade your Node version too. You can do this by installing the
...
<Callout title="Setup via Terminal" type="idea">
...
<Callout type="idea">
  ...
  <code>Ctrl + `</code> to open its integrated terminal.
</Callout>
