...
description: Authenticate app with WhatsApp
---
...
<Callout type="idea">
  For most usage cases, we would recommend the [`LocalAuth` strategy](#localauth-strategy) because
  it is the easiest to use. However, you can also use the [RemoteAuth
  strategy](#remoteauth-strategy) for more flexibility and customization.
</Callout>
...
<Callout title="Warning" type="error">
  `LocalAuth` requires a persistent filesystem to be able to restore sessions. This means that out
  of the box it is not compatible with hosts that provide ephemeral file systems, such as Heroku.
</Callout>
