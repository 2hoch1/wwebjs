...
<Callout type="error">
  You shouldn't assume that the download will be successful. In cases where the media is not able to
  be downloaded (for example, if the media has been deleted from the phone or can no longer be
  downloaded), the `downloadMedia` method will return `undefined`.
</Callout>
